...
// TODO: implement avx512_16
  ...
  template <class T, class A = default_isa> struct vec;
  ...
  // helpers
  template <class T, class A>
  RTS_ALWAYS_INLINE RTS_CONST constexpr const_vref<T,A> make_const_vref(const vec<T,A> & v, std::int32_t i = 0) noexcept {
  ...
  struct vec {
    ...
    RTS_ALWAYS_INLINE constexpr vec(const T & u, step_t) noexcept {
      T v = u;
      for (auto && r : data) r = v++;
    }
...
#undef RTS_ASSIGN
  ...
  struct vec<bool,A> {
    ...
    using difference_type = std::ptrdiff_t;
    using reference = vref<value_type,arch>;
    using const_reference = const_vref<value_type,arch>;
    using pointer = vptr<value_type,arch>;
    using const_pointer = const_vptr<value_type,arch>;
    using iterator = vptr<value_type,arch>;
    using const_iterator = const_vptr<value_type,arch>;
    ...
    RTS_ALWAYS_INLINE constexpr vec(std::uint32_t data, internal_t) noexcept : data(data) {};
    ...
    RTS_ALWAYS_INLINE RTS_PURE constexpr vec operator ~ () const noexcept { return vec(data ^ A::width_mask, internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE constexpr vec operator ! () const noexcept { return vec(data ^ A::width_mask, internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE constexpr vec operator & (const vec & rhs) const noexcept { return vec(data & rhs.data, internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE constexpr vec operator | (const vec & rhs) const noexcept { return vec(data | rhs.data, internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE constexpr vec operator ^ (const vec & rhs) const noexcept { return vec(data ^ rhs.data, internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE constexpr vec operator && (const vec & rhs) const noexcept { return vec(data & rhs.data, internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE constexpr vec operator || (const vec & rhs) const noexcept { return vec(data | rhs.data, internal_tag); }
  ...
  struct vec<bool,avx_4> {
    ...
    template <class U>
    RTS_ALWAYS_INLINE constexpr vec(const vec<U,avx_4> & rhs) noexcept
  ...
  struct vec<bool,avx2_8> {
    ...
    template <class U>
    RTS_ALWAYS_INLINE vec(const vec<U,avx2_8> & rhs) : vec() {
      for (int i=0;i<8;++i)
        if (rhs.get(i))
    ...
    RTS_ALWAYS_INLINE constexpr vec(__m256i m, internal_t) noexcept : m(m) {}
    ...
    RTS_ALWAYS_INLINE RTS_PURE vec operator ! () noexcept { return *this ^ vec(std::true_type()); }
    RTS_ALWAYS_INLINE RTS_PURE vec operator & (const vec & rhs) const noexcept { return vec(_mm256_and_si256(m,rhs.m), internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE vec operator | (const vec & rhs) const noexcept { return vec(_mm256_or_si256(m,rhs.m), internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE vec operator ^ (const vec & rhs) const noexcept { return vec(_mm256_xor_si256(m,rhs.m), internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE vec operator && (const vec & rhs) const noexcept { return vec(_mm256_and_si256(m,rhs.m), internal_tag); }
    RTS_ALWAYS_INLINE RTS_PURE vec operator || (const vec & rhs) const noexcept { return vec(_mm256_or_si256(m,rhs.m), internal_tag); }
    RTS_ALWAYS_INLINE vec & operator = (const vec & rhs) noexcept = default;
    RTS_ALWAYS_INLINE vec & operator = (vec && rhs) noexcept = default;
  ...
  template <>
  struct loader<std::int32_t,avx2_8> : base_loader<std::int32_t,avx2_8> {
  ...
  template <>
  struct loader<float,avx2_8> : base_loader<float,avx2_8> {
    RTS_ALWAYS_INLINE static void load_masked(vector & u, pointers v, mask m) noexcept {
#ifdef RTS_64
  ...
  RTS_ALWAYS_INLINE vec<T,A> load(const vec<T*,A> & pointers, const vec<bool,A> & mask) noexcept {
    vec<T,A> result;
    loader<T,A>::load(result, pointers, mask);
    return result;
  }
  ...
  struct vec<T&,A> {
    ...
    using value_type = T &;
    using base = vec<T*,A>;
    using reference = vrefref<T,A>;
    using pointer = vrefptr<T,A>;
    using iterator = pointer;
    ...
    vec(vec && that) = delete;
    RTS_ALWAYS_INLINE explicit vec(const vec<T*,A> & that, indirection_t) noexcept : pointers(that) {};
    RTS_ALWAYS_INLINE explicit vec(vec<T*,A> && that, indirection_t) noexcept : pointers(std::move(that)) {}
...
#define RTS_CMP(op) \
  ...
  template <class T, class A>
  RTS_ALWAYS_INLINE RTS_PURE constexpr vrefptr<T,A> vrefref<T,A>::operator & () const noexcept {
  ...
  RTS_ALWAYS_INLINE RTS_PURE const vec<T&,A> operator * (const vec<T*,A> & ps) noexcept {
    return vec<T&,A>(ps, indirection_tag);
  }
...
namespace std {
  ...
  template <class T, class A>
  struct iterator_traits<rts::vptr<T,A>> {
    using difference_type = ptrdiff_t;
    using value_type = T;
    using reference_type = rts::vref<T,A>;
    using iterator_category = random_access_iterator_tag;
  };

  template <class T, class A>
  struct iterator_traits<rts::const_vptr<T,A>> {
    using difference_type = ptrdiff_t;
    using value_type = T;
    using reference_type = rts::const_vref<T,A>;
    using iterator_category = random_access_iterator_tag;
  };


  template <class T, class A>
  struct iterator_traits<rts::vrefptr<T,A>> {
    using difference_type = ptrdiff_t;
    using value_type = T;
